...
#Dentro del disco
Ui(r)  = -(15*pi*M)/(128)*(3*η(r)**4+2*η(r)**2+3)
dUi(r) = (15*pi*M*r)/(32*d)*(3*η(r)**2+1)

#Fuera del disco
Uo(r)  = -(15*M)/(64)*((3*ξ(r)**4-2*ξ(r)**2+3)*acot(r)-3*ξ(r)**3+3*ξ(r))
dUo(r) = (15*M)/(16*d*r)*((1-2*ξ(r)**2-3*ξ(r)**4)*acot(r)+3*ξ(r)**3+ξ(r))
...
set output "U2plot_Veff_temp_k20M30.pdf"
...
set output "U2plot_Veff_null_k20M30.pdf"
set key right top box
set xlabel "r̃"
set ylabel "V_{eff}"
set xrange[0:4]
