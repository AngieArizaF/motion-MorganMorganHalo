...
#Dentro del disco
Ui(r)  = -(35*pi*M)/(512)*(5*η(r)**6+3*η(r)**4+3*η(r)**2+5)
dUi(r) = (35*pi*M*r)/(256*d)*(15*η(r)**4+6*η(r)**2+3)

#Fuera del disco
Uo(r)  = -(35*M)/(256)*((-5*ξ(r)**6+3*ξ(r)**4-3*ξ(r)**2+5)*acot(r)+5*ξ(r)**5-14./3*ξ(r)**3+5*ξ(r))
dUo(r) = (35*M)/(128*d*r)*((3-3*ξ(r)**2+9*ξ(r)**4+15*ξ(r)**6)*acot(r)+ξ(r)*(3-4*ξ(r)**2-15*ξ(r)**4))
...
set output "U3plot_Veff_temp_k20M30.pdf"
...
set output "U3plot_Veff_null_k20M30.pdf"
set key right top box
set xlabel "r̃"
set ylabel "V_{eff}"
set xrange[0:4]
